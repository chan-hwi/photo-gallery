...
import type { CommentType, ReplyPageType } from '@/types/models';
...
function Comment({ comment, postId, style, sx } : paramType) {
  const { user } = useUser();
  const queryClient = useQueryClient();
  const [showCommentInput, setShowCommentInput] = useState<boolean>(false);
  const [showUpdateCommentInput, setShowUpdateCommentInput] = useState<boolean>(false);
  ...
  const {
    ...
    async ({ pageParam = 1 }) => {
      const res = await api.get<ReplyPageType>(`/comments/${comment._id}/replies`, {
        params: {
          page: pageParam,
        },
      });
  ...
  const handleChange = useCallback((e : React.ChangeEvent<HTMLInputElement>) => setCommentInput(e.target.value), []);
  const handleSubmit = () => {
  ...
  const handleChangeUpdateCommentInput = useCallback((e : React.ChangeEvent<HTMLInputElement>) => {
              ...
              >
                <Tooltip title="Like this comment">
                  <IconButton onClick={handleLike}>
                    {user && comment.likes.includes(user?.profile?.id) ? (
                      <Favorite />
                    ) : (
                      <FavoriteBorder />
                    )}
