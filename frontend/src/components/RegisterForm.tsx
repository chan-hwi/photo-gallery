...
import type { UserFormType } from "types/models";

const initialUser : UserFormType = {
    username: "",
    password: "",
    nickname: "",
    email: "",
...
function RegisterForm() {
  ...
  const [usrErrMsg, setUsrErrMsg] = useState<string>("");
  const [pwErrMsg, setPwErrMsg] = useState<string>("");
  const [cpErrMsg, setCpErrMsg] = useState<string>("");
  ...
  const handleSubmit = async (e : React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isAxiosError(err) && err?.response?.status === 403) setUsrErrMsg(err?.response?.data?.message);
